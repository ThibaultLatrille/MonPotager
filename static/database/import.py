...
with open('especes.csv', newline='', encoding='utf-8') as file:
    ...
    for row in reader:
        ...
        cursor.execute("INSERT INTO specie (category, calendar) VALUES(%s, %s::bytea) RETURNING id",
                       (category, get_calendar_binary_string(row[5:])))
        ...
        cursor.execute("INSERT INTO media (hash, url, type) VALUES(%s,%s, 'R')",
                       (hashlib.sha256(str.encode(row[4])).digest(), row[4]))
        pg.commit()
...
with open('associations.csv', newline='', encoding='utf-8') as file:
    ...
    for row in reader:
        ...
        cursor.execute("INSERT INTO interaction (beneficial,specie1,specie2, sources) VALUES(%s, %s, %s, %s)",
                       (t, species[row[0]], species[row[2]], json.dumps(row[3].strip() + ', ' + row[5].strip())))
        pg.commit()
